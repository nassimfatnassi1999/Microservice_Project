...
//    }
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable) // Disable CSRF
                .authorizeHttpRequests((requests) -> requests
                        ///.requestMatchers("/user/register_user","/user/authenticate_user","/user/getbyid/**","/user/resetpasswordrequest","/user/resetpassword").permitAll() // Always allowed
                        //.requestMatchers("/user/getbytoken").authenticated() // authentication only
                        //.requestMatchers("/admin/**").hasRole("ADMIN") // needs admin role
                        .anyRequest().permitAll()//.authenticated() // the rest requires authentication
